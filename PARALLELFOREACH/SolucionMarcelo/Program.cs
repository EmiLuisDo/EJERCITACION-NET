    ...
    class Matriz
    {
        static void Main(string[] args)
        {
            ...
            */
            ...
            List<double[,]> resultado = muliplicarMatrizParalelo(arrayList, mB);

            stopwatch.Stop();
            Console.Error.WriteLine("Tiempo for Paralelo (segundos): {0}", stopwatch.ElapsedMilliseconds / 1000);

            /*foreach (double[,] resultP in resultado)
            {
                Console.WriteLine("Entra");
                //imprimirMatriz( cantColumnasB, cantFilasA, resultP);
                Console.WriteLine("Imprimiendo filas=" + resultP.GetLength(0));
                Console.WriteLine("Imprimiendo columnas=" + resultP.GetLength(1));
            }*/
        ...
        static List<double[,]> muliplicarMatrizParalelo(List<Double[,]> array, double[,] mB)
        {
            ...
            {
                Console.WriteLine("Entra");
                Double[,] resultS = muliplicarMatrizSecuencial(element, mB);
                resultado.Add(resultS);
            }
            );
